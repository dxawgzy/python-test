...
def attach_volume(TOKEN,values,tenant_id,para,para_id):
    url = 'http://10.89.151.11:8776/v2/%s/%s/%s/action' %(tenant_id,para+'s',para_id)
    params = json.dumps(values)
    headers = {"Content-type":"application/json","Accept": "application/json"}
    req = urllib2.Request(url, params, headers)
    req.add_header("X-Auth-token",TOKEN)
...
def detach_volume(TOKEN,values,tenant_id,para,para_id):
    url = 'http://10.89.151.11:8776/v2/%s/%s/%s/action' %(tenant_id,para+'s',para_id)
    params = json.dumps(values)
    headers = {"Content-type":"application/json","Accept": "application/json"}
    req = urllib2.Request(url, params, headers)
...
if __name__ == '__main__':
    ...
    if status == 'ACTIVE':
        ...
        time.sleep(15)
        values6 = {"os-attach": {"instance_uuid": instance_id , "mountpoint": "/dev/vdb"}}
        attach_volume(TOKEN,values6,tenant_id,'volume',volume_id)

    """
    values7 = {"os-force_attach": {"attachment_id": "", "": {""}}}
    detach_volume(TOKEN,values7,tenant_id,'volume',volume_id)
