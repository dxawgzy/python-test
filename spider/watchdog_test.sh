...
function check_status_normal()
{
    ser=$1
    result=`service $ser status | grep Active | awk '{print $2}'`
    if [ "$result" = "active" ]
    then
        echo "OK. Now $ser status is active."
    else
        echo "Fail. Now $ser status is stopped." >> $file
    fi
}

# When stop/kill service, status should be stopped.
function check_status_stopped()
{
    ser=$1
    result=`service $ser status | grep Active | awk '{print $2}'`
    if [ "$result" = "active" ]
    then
        echo "Fail. After stop $ser service, status is not stopped." >> $file
    else
        echo "OK. Now $ser status is stopped."
    fi
}
...
function check_process_stopped()
{
    ...
    if [ -n "$result" ]
    then
        echo "Fail. After kill $ser , process is not stopped." >> $file
    else
...
function stop_service()
{
    ...
    sleep 6
    check_status_stopped $serv_list
}
...
{
    serv_list=$1
    for ser in ${serv_list[*]}
    do
        check_status_normal $ser
    done
}

function check_all_process()
{
    proc_list=$1
...
do
    #check_status_normal ${contr_serv[$i]}
    stop_service ${contr_serv[$i]}
done
sleep 60
...
for ((i=0; i<$contr_lens; i++))
do
    kill_process ${contr_serv[$i]} ${contr_proc[$i]}
done
sleep 60
echo "=====After test, check all service process.=====" | tee -a $file
check_all_process "${contr_proc[*]}"
...
do
    #check_status_normal ${comp_serv[$i]}
    stop_service ${comp_serv[$i]}
done
sleep 30
...
for ((i=0; i<$comp_lens; i++))
do
    kill_process ${comp_serv[$i]} ${comp_proc[$i]}
done
sleep 30
echo "=====After test, check all service process on $compute_ip.=====" | tee -a $file
check_all_process "${comp_proc[*]}"
fi
...
do
    #check_status_normal ${net_serv[$i]}
    stop_service ${net_serv[$i]}
done
sleep 30
...
for ((i=0; i<$net_lens; i++))
do
    kill_process ${net_serv[$i]} ${net_serv[$i]}
done
sleep 30
echo "=====After test, check all service process on $network_ip.=====" | tee -a $file
check_all_process "${net_serv[*]}"
fi
