...
# http://blog.csdn.net/kangvcar/article/details/78256751
# https://github.com/kangvcar/pyproject/blob/master/Spiders/Spider_xixizhan.py
...
# 原程序报错 httplib.IncompleteRead: IncompleteRead(259 bytes read, 7933 more expected)
# 如下为解决方法
import httplib
...
class getMovieInfo(object):
	...
	def pipeline2mysql(self, items):
		...
		for item in items:
			...
			cur.execute("INSERT INTO ygdy8(name, type, time, link) values(%s,%s,%s,%s)",
						(mtitle, mtype, mtime, downloadlink))
		cur.close()
		conn.commit()
		conn.close()
