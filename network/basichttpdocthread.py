...
class RequestHandler(BaseHTTPRequestHandler):
    """Definitin of the request handler. """
    def _writeheaders(self, doc):
        """Write the HTTP headers for the document. If there is no
        document, send a 404 e4rror code; otherwise, send a 200 success code."""
        if doc is None:
    ...
    def _getdoc(self, filename):
        """Handle a requesst for a document, returning one of two
